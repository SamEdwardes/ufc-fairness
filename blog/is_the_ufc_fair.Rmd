...
But I do not know if `r test_accuracy * 100`% consistency is good enough. For example how would the judges explain the results of these fights which were the biggest statistical anamolies?
...
ufc_out %>%
  ...
  head(5) %>%
  select(event_name_fighters, blue, red, colour_win:red_pass, prediction_proba, correct) %>%
  select(-blue_name, -red_name) %>%
  rename(event = event_name_fighters, winner = colour_win) %>%
  knitr::kable()
```

What happened in these fights that a fighter was able to win while performing poorly on striking, take-downs, and passes? Is it poor judging, or something that can't be observed in the data we have?
