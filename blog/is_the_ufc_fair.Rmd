...
When a UFC match ends with a knock-out or submission there is usually never any doubt who the better fighter was. But only `r round(percent_finish * 100)`% of fights end so decisively. The other `r round(percent_judges * 100)`% of fights go to the judges score cards.

```{r win types plot}
fig_win_types
```

At least when the fights do go to the judges scorecard, it appears that the judges are usually pretty confident. The judges are unanimous in their decision `r round(percent_unanimous * 100)`% of the time.

```{r}
fig_judge_outcomes
```

But, judging is tricky business. It is frequently the subject of public scrutiny:
...
The system is somewhat confusing, but here are the basics. Judges score each round on a "*10-Point Must System*":
...
- The next most important features associated with losing are also passes, then take-downs.
...
Personally, I think this analysis provides some comfort that the judging is not terrible. If you land more strikes, you will probably win. If you earn more take-downs, and do a better job of passing guard you will probably win. 
...
For example here are the 5 fights that the model was most confident about, but predicted the wrong result.
...
ufc_out %>%
  ...
  mutate(prediction_proba = if_else(prediction_proba > 0.5, 
         ...
         prediction = if_else(prediction == 1, "blue", "red"),
         blue = if_else(colour_win == "blue", fighter_1_name, fighter_2_name),
         red = if_else(colour_win == "red", fighter_1_name, fighter_2_name)) %>%
  arrange(desc(prediction_proba)) %>%
  head(5) %>%
  select(event_name_fighters, blue, red, colour_win:correct) %>%
  knitr::kable()
```

What happened in these fights that the fighter was able to win, while performing poorly on striking, take-downs, and passes? Is it poor judging, or something that can't be observed in the data we have?
...
- The logistic regression model is useless for picking fights before they occur, because it is using data that can only be observed after the fight occurred. So it cannot help any gamblers out there looking for an edge.
